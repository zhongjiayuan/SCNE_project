...
data=pd.read_csv('KIRC_matrix.csv', sep = ',',header=None,index_col=0)
all_gene=list(data.index)



f=open('KIRC_network.txt')
node_center=[]
nodes_nei=[]
data_matrix=[]
while True:
...
def  re(sample_test,gene_name,predict_pairs1):
    ...
    for i in range(len(gene_name)):
        ...
        sm=abs((sample_test[0][i]-np.mean(train[:,i]))/(np.std(train[:,i])+es))**2
...
if __name__=="__main__":
    ...
    for ii in range(len(test)):
        ...
        for i in gene_name:
            ...
            rt.append(regu)
        text_save(path0+path_name[0][0:-4]+'_KIRC_pcc0_r.txt',rt)
        f = codecs.open(path0+path_name[0][0:-4]+'_KIRC_pcc0_r.txt', mode='r', encoding='utf-8')
        line = f.readline() 
        targets = []
        regulators=[]
        while line:
        ...
        f.close()
        print(ii)
        filename=path0+path_name[0][0:-4]+'_KIRC_pcc0_r.txt'
        predict_pairs1=pairs1(path0+path_name[0][0:-4]+'_KIRC_pcc0_r.txt')
